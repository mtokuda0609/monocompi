class ItemsController < ApplicationController
  def index
    @items = Item.where(user_id: current_user).order(created_at: :desc).page(params[:page]).per(10)
  end
  ...
  def create
    ...
    if name == "" || rk_code == "" || ys_code == ""
      @item = Item.new
      flash.now[:danger] = "アイテムを選択し、アイテム名を入力してください。"
      render :new
    else
  ...
  def item_params
    params.require(:item).permit(:name, :user_id, :rk_code, :ys_code)
  end
